...
function checkLogin() {
	if (!isset($_SESSION['id_user'])) {
		setAlert("Akses Ditolak!", "Login Dahulu!", "error");
...
function checkLoginAtLogin() {
	if (isset($_SESSION['id_user'])) {
		setAlert("Anda Telah Login!", "Selamat Datang!", "Berhasil");
		header('Location: index.php');
	}
}

function logout() {
	setAlert("Anda Telah Logout!", "Berhasil Logout!", "Berhasil");
	header("Location: login.php");
}
...
function checkJabatan() {
	...
	if ($id_jabatan !== '1') {
		setAlert("Akses Ditolak!", "Anda tidak dapat menghapus data selain administrator!", "error");
     	header("Location: index.php");
	} else {
		return true;
	}
...
function addUser($data) {
	...
	if (mysqli_fetch_assoc($query)) {
		setAlert("Gagal menambahkan user!", "Username sudah digunakan!", "error");
     	return header("Location: user.php");
	} else {
		$password = htmlspecialchars($data['password']);
		$password_verify = htmlspecialchars($data['password_verify']);
		if ($password !== $password_verify) {
			setAlert("Gagal menambahkan user!", "password tidak sesuai!", "error");
	     	return header("Location: user.php");
		} else {
...
function addBulanPembayaran($data) {
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		setAlert("Gagal menambahkan " . ucwords($nama_bulan) . "!", "karena " . ucwords($nama_bulan) . " telah terdaftar!", "error");
...
function changePassword($data) {
	...
	if (password_verify($old_password, $dataUser['password'])) {
		...
		if ($new_password == $new_password_verify) {
			...
			setAlert("Gagal mengubah password!", "Password baru tidak sesuai!", "error");
	     	return header("Location: profile.php");
		}
	} else {
		setAlert("Gagal mengganti password!", "Password lama tidak sesuai!", "error");
     	return header("Location: profile.php");
	}
}
