<?php 
  ...
  $lastMonth = date("Y-m", strtotime("-1 month", strtotime($firstDayOfMonth)));
  $newYear = date("Y", strtotime($today));
  $newMonth = date("m", strtotime($today));
  
  $monthColumn = 'nama_bulan';
  $yearColumn = 'tahun';
  
  $bulan = mysqli_real_escape_string($conn, date("F", strtotime($today))); // Assuming that 'nama_bulan' is the full month name.
  
  // Check if the entry for the current month and year already exists
  $existingMonth = mysqli_query($conn, "SELECT * FROM bulan_pembayaran WHERE $monthColumn='$bulan' AND $yearColumn=$newYear");
  
  if (mysqli_num_rows($existingMonth) == 0) {
      $newMonthData = [
          'nama_bulan' => $bulan,
          'tahun' => $newYear,
      ...
      // Pastikan fungsi ini sesuai dengan kebutuhan Anda
      addBulanPembayaran($newMonthData);
  }  
  ...
  if (isset($_POST['btnAddBulanPembayaran'])) {
    if (addBulanPembayaran($_POST) > 0) {
      setAlert("Bulan Pembayaran Telah Ditambahkan", "Berhasil ditambahkan", "Berhasil");
  ...
  if (isset($_POST['btnEditBulanPembayaran'])) {
    if (editBulanPembayaran($_POST) > 0) {
      setAlert("Bulan Pembayaran Telah Diubah", "Berhasil diubah", "Berhasil");
      header("Location: uang_kas.php");
    }
  }
?>
